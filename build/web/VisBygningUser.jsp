<%-- 
    Document   : VisBygningUser
    Created on : 11-11-2016, 05:08:31
    Author     : JonasSchack
--%>
<%
String LoginType = (String) request.getSession().getAttribute("user_type");
String FirmName = (String) request.getSession().getAttribute("firm_name");
...
for (int i = 0; i < buildings.size(); i++) {
            ...
            if (uID == buildings.get(i).getUserID()) {
...
%>
        ...
        <form method = "post" action="CreateBuilding">
                    <td><a href="BimgHandler?id=<%out.print(buildings.get(i).getBuildingID());%>">
                    <img src="BimgHandler?id=<%out.print(buildings.get(i).getBuildingID());%>" height="160" width="160"/></a></td>
                    <td><%out.print(FirmName);%></td>
                    <td><%out.print(buildings.get(i).getStreet());%></td>
                    <td><%out.print(zip.get(id).getCity());%><br><%out.print(zip.get(id).getZipcodes());%></td>
                    <td><%out.print(buildings.get(i).getM2());%></td>
                    <td><%out.print(buildings.get(i).getFloors());%></td>
                    <td><select name="valg">
                    <option value="visRapport">Vis rapport</option>
